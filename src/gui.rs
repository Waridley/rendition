...
use killcam_app::{KillcamPlugin, KillcamWorld, killcam_active};
use sim::{Sim, SimMain};
...
impl Plugin for GuiPlugin {
	fn build(&self, app: &mut App) {
		app.add_plugins((DefaultPlugins, ClientPlugin, KillcamPlugin))
			.add_systems(
				FixedUpdate,
				(run_client, run_killcam.run_if(killcam_active)),
			)
